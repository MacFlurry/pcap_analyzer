...
async def test_complete_workflow_upload_to_report(client, sample_pcap_file, test_db, db_type):
    ...
    with open(sample_pcap_file, "rb") as f:
        upload_response = client.post("/api/upload", files={"file": ("test.pcap", f, "application/vnd.tcpdump.pcap")})
...
def test_upload_and_check_queue(client, sample_pcap_file):
    ...
    # Upload
    with open(sample_pcap_file, "rb") as f:
        client.post("/api/upload", files={"file": ("test.pcap", f, "application/vnd.tcpdump.pcap")})
...
def test_multiple_uploads_sequential(client, sample_pcap_file):
    ...
    for i in range(3):
        with open(sample_pcap_file, "rb") as f:
            response = client.post("/api/upload", files={"file": (f"test_{i}.pcap", f, "application/vnd.tcpdump.pcap")})
