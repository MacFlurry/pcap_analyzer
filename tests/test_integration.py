...
class TestAnalyzerIntegration:
    ...
    def test_rtt_measurement_with_data_exchange(self):
        ...
        # Client sends data
        pkt1 = Ether() / IP(src="192.168.1.100", dst="192.168.1.1") / TCP(
            sport=12345, dport=80, flags="PA", seq=1000, ack=2000
        ) / b"REQUEST"
        pkt1.time = 1.0
        packets.append(pkt1)

        # Server ACKs
        pkt2 = Ether() / IP(src="192.168.1.1", dst="192.168.1.100") / TCP(
            sport=80, dport=12345, flags="A", seq=2000, ack=1007  # ACK seq+len
        )
