...
async def test_progress_updates_storage(test_worker):
    ...
    update = ProgressUpdate(
        task_id=task_id, phase="metadata", progress_percent=50, packets_processed=1000, total_packets=2000
    )
...
async def test_clear_progress_updates(test_worker):
    ...
    # Ajouter un update
    test_worker.progress_updates[task_id].append(ProgressUpdate(task_id=task_id, phase="test", progress_percent=0))
