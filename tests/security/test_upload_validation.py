...
def test_path_traversal_attempt(client: TestClient, test_data_dir):
    ...
    malicious_file.write_bytes(bytes.fromhex(
        "d4c3b2a10200040000000000000000000000ffff000001000000"
    ))
    ...
    with open(malicious_file, "rb") as f:
        response = client.post(
            "/api/upload",
            files={"file": ("../../../etc/passwd.pcap", f, "application/vnd.tcpdump.pcap")}
        )
...
def test_sql_injection_in_filename(client: TestClient, sample_pcap_file):
    ...
    with open(sample_pcap_file, "rb") as f:
        response = client.post(
            "/api/upload",
            files={"file": ("test'; DROP TABLE tasks; --.pcap", f, "application/vnd.tcpdump.pcap")}
        )
...
def test_xss_in_filename(client: TestClient, sample_pcap_file):
    ...
    with open(sample_pcap_file, "rb") as f:
        response = client.post(
            "/api/upload",
            files={"file": ("<script>alert('xss')</script>.pcap", f, "application/vnd.tcpdump.pcap")}
        )
...
def test_null_byte_injection(client: TestClient, sample_pcap_file):
    ...
    with open(sample_pcap_file, "rb") as f:
        response = client.post(
            "/api/upload",
            files={"file": ("test\x00.txt.pcap", f, "application/vnd.tcpdump.pcap")}
        )
...
def test_multiple_extensions(client: TestClient, test_data_dir):
    ...
    multi_ext_file.write_bytes(bytes.fromhex(
        "d4c3b2a10200040000000000000000000000ffff000001000000"
    ))
    ...
    with open(multi_ext_file, "rb") as f:
        response = client.post(
            "/api/upload",
            files={"file": ("file.exe.pcap", f, "application/vnd.tcpdump.pcap")}
        )
...
def test_oversized_filename(client: TestClient, sample_pcap_file):
    ...
    with open(sample_pcap_file, "rb") as f:
        response = client.post(
            "/api/upload",
            files={"file": (long_name, f, "application/vnd.tcpdump.pcap")}
        )
