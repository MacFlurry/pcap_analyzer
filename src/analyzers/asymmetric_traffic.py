...
class AsymmetricTrafficAnalyzer:
    ...
    def __init__(self, asymmetry_threshold: float = 0.3, 
                 min_bytes_threshold: int = 10000,
                 min_packets_threshold: int = 10):
        """
    ...
    def process_packet(self, packet: Packet, packet_num: int = 0) -> None:
        ...
        flow_key, is_forward = self._normalize_flow_key(
    ...
    def get_results(self) -> Dict[str, Any]:
        ...
        symmetric_flows = [f for f in self.flows.values() 
                          if f.byte_ratio() >= 0.8 and 
                          f.total_bytes() >= self.min_bytes_threshold]
