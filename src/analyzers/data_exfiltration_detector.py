...
class DataExfiltrationDetector(BaseAnalyzer):
    ...
    def __init__(
        ...
        self.suspicious_ports = suspicious_ports or [
            ...
            8888,
            9000,
            9001,  # Alternative HTTP/HTTPS
            3128,
            8123,  # Proxy ports
            4444,
            5555,
            6666,
            7777,  # Common backdoor ports
            31337,
            12345,  # Classic backdoor ports
        ]

        # Track upload/download per IP
        self.traffic_by_ip: dict[str, dict[str, Any]] = defaultdict(
            lambda: {
                "upload_bytes": 0,
                "download_bytes": 0,
                "upload_packets": 0,
    ...
    def _record_upload(self, src_ip: str, dst_ip: str, size: int, timestamp: float, pkt: Any):
    ...
    def _detect_large_uploads(self):
        ...
        for src_ip, windows in self.upload_windows.items():
            ...
            for i, (start_time, _) in enumerate(windows):
                ...
                window_bytes = sum(size for ts, size in windows[i:] if start_time <= ts <= end_time)
                    ...
                    self.exfiltration_events.append(
                        {
                            ...
                            "ports_used": sorted(data["ports_used"]),
    ...
    def _detect_unusual_protocols(self):
        ...
        for src_ip, data in self.traffic_by_ip.items():
            ...
            suspicious_ports_used = [port for port in data["ports_used"] if port in self.suspicious_ports]
