...
class ThroughputAnalyzer:
    ...
    def _calculate_throughput(self, bytes_count: int, first_ts: float, last_ts: float) -> dict[str, float]:
        ...
        if not first_ts or not last_ts:
            return {"duration_seconds": 0, "bytes_per_second": 0, "kbps": 0, "mbps": 0}
    ...
    def get_results(self) -> dict[str, Any]:
        ...
        # Throughput global
        global_throughput = self._calculate_throughput(self.total_bytes, self.first_packet_time, self.last_packet_time)

        # Throughput par flux
        flow_stats = []
        for flow_key, flow in self.flows.items():
            throughput = self._calculate_throughput(flow["bytes"], flow["first_timestamp"], flow["last_timestamp"])
        ...
        slow_flows = [
            f for f in flow_stats if f["duration_seconds"] > 1.0 and f["throughput_mbps"] < 1.0 and f["bytes"] > 10000
        ]
