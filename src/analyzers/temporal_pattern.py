...
class TemporalPatternAnalyzer:
    ...
    def _detect_periodicity(self, timestamps: List[float], 
                           min_occurrences: int = 5) -> List[PeriodicPattern]:
        """
    ...
    def _get_hourly_distribution(self) -> Dict[int, Dict[str, Any]]:
        ...
        hourly: Dict[int, Dict[str, int]] = defaultdict(
            lambda: {"packets": 0, "bytes": 0, "slots": 0}
        )
    ...
    def get_summary(self) -> str:
        ...
        if results["periodic_patterns"]:
            ...
            for p in results["periodic_patterns"][:3]:
                lines.append(f"  - {p['source_ip']}: toutes les {p['description']} "
