...
class SackAnalyzer:
                ...
                if kind == 5 and data:
                    ...
                    for i in range(num_blocks):
                        ...
                        try:
                            ...
                            left_edge, right_edge = struct.unpack("!II", data[offset : offset + 8])
    ...
    def _is_dsack(self, sack_blocks: List[SackBlock], ack_num: int) -> Tuple[bool, Optional[int]]:
        ...
        if len(sack_blocks) > 1:
            ...
            if first_block.left_edge >= second_block.left_edge and first_block.right_edge <= second_block.right_edge:
                return True, first_block.left_edge
    ...
    def process_packet(self, packet: Packet, packet_num: int = 0) -> None:
        ...
        if flow_key not in self.flows:
            self.flows[flow_key] = FlowSackStats(
                flow_key=flow_key, src_ip=src_ip, dst_ip=dst_ip, src_port=src_port, dst_port=dst_port
            )
    ...
    def get_results(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "efficiency": {
                "avg_sacked_bytes_per_event": (
                    round(total_sacked_bytes / len(self.sack_events), 0) if self.sack_events else 0
                ),
                "flows_with_dsack": len(dsack_flows),
                "dsack_flows_percentage": (
                    round(len(dsack_flows) / flows_using_sack * 100, 1) if flows_using_sack > 0 else 0
                ),
            ...
            "top_sack_flows": [
                {
                    ...
                    "first_sack": (
                        datetime.fromtimestamp(f.first_sack_time).strftime("%H:%M:%S") if f.first_sack_time else ""
                    ),
                    "duration_seconds": (
                        round(f.last_sack_time - f.first_sack_time, 1) if f.first_sack_time and f.last_sack_time else 0
                    ),
                }
                for f in top_flows
            ],
    ...
    def get_summary(self) -> str:
        ...
        if efficiency["flows_with_dsack"] > 0:
            lines.append(
                f"⚠️  Flux avec D-SACK: {efficiency['flows_with_dsack']} ({efficiency['dsack_flows_percentage']}%)"
            )
