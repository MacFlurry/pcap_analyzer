...
class PasswordResetService:
    ...
    async def create_reset_token(self, user_id: str, ip_address: Optional[str] = None, user_agent: Optional[str] = None) -> str:
        """
    ...
    async def validate_token(self, token_hash: str) -> Optional[User]:
        ...
        if not row:
            logger.warning(f"PASSWORD_RESET_VALIDATION_FAILED: token not found or already used, hash={token_hash[:8]}...")
            return None
